...
/* =========================================================
   ...
   Dark appearance is permanent; only motion preference remains.
========================================================= */
(() => {
  'use strict';
  ...
  root.dataset.motion = savedMotion === 'off'
    ? 'off'
    : (savedMotion === 'on'
      ? 'on'
  ...
  window.MadhiPreferences = { motionEnabled, applyMotion, syncAllEmbeddedMotion, setupPreferenceControls };
})();
...
function mountSiteShell() {
  ...
  const onLandingPage = document.documentElement.dataset.page === "home";
  const landingHomeHref = onLandingPage ? "#home" : siteUrl(`${landingFile}#home`);
  const landingContactHref = onLandingPage ? "#contact" : siteUrl(`${landingFile}#contact`);
  ...
  if (navMount) {
    navMount.innerHTML = `
      <nav class="site-nav-global" aria-label="Primary navigation">
        <div class="nav-inner">
          ...
          <div class="nav-links" id="primaryNavLinks">
            <a href="${landingHomeHref}" data-nav="home">Home</a>
      ...
      <div class="site-preferences" aria-label="Display settings">
        <button class="preference-toggle back-to-top-toggle" id="backToTop" type="button" aria-label="Return to top of page"><span class="back-to-top-arrow" aria-hidden="true">↑</span><span class="back-to-top-label" aria-hidden="true">Return to top</span></button>
  ...
  const pageKind = document.documentElement.dataset.page || "";
  if (pageKind === "service" && !document.querySelector(".mobile-brand-watermark")) {
...
(() => {
  ...
  syncMobileBrandState();
  window.addEventListener('scroll', syncMobileBrandState, { passive: true });
  mobileQuery.addEventListener?.('change', syncMobileBrandState);
})();
...
/* Landing navigation state.
   On the landing page, only the section currently crossing the viewport center is
   highlighted. Home is active only when no service/contact section is current.
   Dedicated service pages keep only their own page pill active. */
(() => {
  ...
  function updateNavigationState() {
    ...
    if (!currentId && contactSection) {
      ...
      if (rect.top <= probeY && rect.bottom > probeY) currentId = 'contact';
    }

    serviceNavLinks.forEach(link => {
      link.classList.toggle('section-current', currentId === link.dataset.sectionTarget);
    });
    contactNavLink?.classList.toggle('section-current', currentId === 'contact');
    homeNavLink?.classList.toggle('active', !currentId);
  }

  function queueNavigationStateUpdate() {
    if (navStateFrame) return;
    navStateFrame = window.requestAnimationFrame(updateNavigationState);
  }
...
(() => {
  ...
  function setupSoccer(){
    ...
    if(!host||!svgs.length||!button)return;
    ...
    const mapPosition=(position,mobile)=>mobile?[277-position[1],position[0]]:[position[0],position[1]];
    const mkText=(x,y,text,anchor='middle',size='8',color='#fff',opacity='1')=>{
      ...
      t.setAttribute('font-family','Arial,sans-serif');t.setAttribute('font-size',size);
      t.setAttribute('font-weight','700');t.setAttribute('fill',color);t.setAttribute('opacity',opacity);
    ...
    const pitches=svgs.map(svg=>{
      ...
      layer.classList.add('formation-layer');
      svg.appendChild(layer);
      const players=[];
      const addTeam=(team,color,positions)=>{
        positions.forEach((pos,i)=>{
          const mapped=mapPosition(pos,mobile);
          const g=document.createElementNS(NS,'g');g.dataset.team=team;g.dataset.index=i;
          const c=document.createElementNS(NS,'circle');
          c.setAttribute('r',i===0?'7':'6');c.setAttribute('fill',color);c.setAttribute('stroke','#fff');c.setAttribute('stroke-width','1.6');
          g.appendChild(c);
          if(pos[2])g.appendChild(mkText(0,2.7,pos[2]));
          layer.appendChild(g);
          players.push({g,team,index:i,x:mapped[0],y:mapped[1]});
          g.setAttribute('transform',`translate(${mapped[0]} ${mapped[1]})`);
        });
      };
      ...
      if(mobile){
        addLabel(layer,{x:142.5,y:13,text:'4-3-3'},'#ff3b3b','16');
        addLabel(layer,{x:142.5,y:490,text:'3-5-2'},'#2563eb','16');
      }else{
        [
          {x:88,y:21,text:'4'},
          {x:119,y:21,text:'-'},
    ...
    function animateTo(targetRed,targetBlue){
      ...
      const targetsFor=(pitch,p)=>mapPosition((p.team==='red'?targetRed:targetBlue)[p.index],pitch.mobile);
      if(window.MadhiPreferences?.motionEnabled() === false){
        pitches.forEach(pitch=>pitch.players.forEach(p=>{
          const target=targetsFor(pitch,p);p.x=target[0];p.y=target[1];
          p.g.setAttribute('transform',`translate(${p.x} ${p.y})`);
        }));
        return;
      }
      const start=performance.now();const duration=950;
      const state=pitches.map(pitch=>({
        pitch,
        starts:pitch.players.map(p=>({x:p.x,y:p.y})),
        targets:pitch.players.map(p=>targetsFor(pitch,p))
      }));
      const ease=t=>1-Math.pow(1-t,3);
      const frame=now=>{
        const t=Math.min(1,(now-start)/duration),e=ease(t);
        state.forEach(({pitch,starts,targets})=>pitch.players.forEach((p,i)=>{
          const tx=targets[i][0],ty=targets[i][1];
          const x=starts[i].x+(tx-starts[i].x)*e;
          const y=starts[i].y+(ty-starts[i].y)*e;
          p.g.setAttribute('transform',`translate(${x} ${y})`);
          if(t===1){p.x=tx;p.y=ty;}
        }));
    ...
    button.addEventListener('click',()=>{
      ...
      button.textContent=facing?'Reset Formations':'Blow Whistle';
      button.classList.toggle('is-active',facing);
    });
  }
...
/* V9: hero photograph is permanently visible; legacy scroll-fade controller removed. */



/* Mobile disclosure coordination: keep only one detail panel open at a time. */
function closeOtherMobileDisclosurePanels(exceptPanel) {
  document.querySelectorAll('.mobile-service-accordion.is-expanded').forEach(wrapper => {
    if (wrapper === exceptPanel) return;
    wrapper.classList.remove('is-expanded');
    const button = wrapper.querySelector('.landing-indexed-heading .mobile-disclosure-toggle');
    const card = wrapper.querySelector('.sitemap-card');
    const copy = wrapper.querySelector('.service-accordion-copy');
    const feature = wrapper.querySelector('.service-accordion-feature');
  ...
  document.querySelectorAll('.approach-viewport .studio-info-card.is-expanded, .client-viewport .studio-info-card.is-expanded').forEach(card => {
    if (card === exceptPanel) return;
    card.classList.remove('is-expanded');
    card.querySelector('.mobile-disclosure-toggle')?.setAttribute('aria-expanded', 'false');
    card.querySelector('.studio-accordion-body')?.setAttribute('aria-hidden', 'true');
  });
}

function preserveMobileDisclosureAnchor(anchor, update) {
  if (!anchor || typeof update !== 'function') {
    update?.();
    return;
  }
  ...
  const holdAnchor = startTime => {
    ...
    if (performance.now() - startTime < 360) {
      window.requestAnimationFrame(() => holdAnchor(startTime));
    }
  };

  const startTime = performance.now();
  holdAnchor(startTime);
}
...
(() => {
  ...
  function ensureServiceAccordion(section, card, wrapper) {
    ...
    copy.classList.add('service-accordion-copy');
    feature.classList.add('service-accordion-feature');

    let button = heading.querySelector('.mobile-disclosure-toggle');
    if (!button) {
      button = document.createElement('button');
      button.type = 'button';
      button.className = 'mobile-disclosure-toggle';
      button.setAttribute('aria-expanded', 'false');
      button.setAttribute('aria-label', `Reveal ${heading.querySelector('.section-title')?.textContent?.trim() || 'section'} details`);
    ...
    accordionReady.add(wrapper);
  }

  function createWrapper(id) {
    const wrapper = document.createElement('div');
    wrapper.className = 'mobile-service-accordion';
    wrapper.dataset.serviceAccordion = id;
    wrappers.set(id, wrapper);
    return wrapper;
  }

  function arrangeMobileServices() {
    const grid = document.querySelector('#services .sitemap-grid');
    if (!grid) return;
    const cards = [...grid.querySelectorAll('.sitemap-card')];
    ids.forEach((id, index) => {
      const section = document.getElementById(id);
      const card = cards[index];
      if (!section || !card) return;
      if (!placeholders.has(section)) {
        const marker = document.createComment(`restore-${id}`);
        section.parentNode.insertBefore(marker, section);
        placeholders.set(section, marker);
      }
      let wrapper = wrappers.get(id);
      if (mobileQuery.matches) {
        if (!wrapper) wrapper = createWrapper(id);
        if (!wrapper.isConnected) card.before(wrapper);
        wrapper.append(section);
        const feature = section.querySelector('.feature-layout');
        if (feature) feature.before(card);
        ensureServiceAccordion(section, card, wrapper);
      } else {
        if (wrapper?.isConnected) {
          wrapper.before(card);
          wrapper.remove();
        }
        const copy = card.querySelector('.service-accordion-copy');
        const feature = section.querySelector('.service-accordion-feature');
        card.hidden = false;
        if (copy) copy.hidden = false;
        if (feature) feature.hidden = false;
        const button = section.querySelector('.landing-indexed-heading .mobile-disclosure-toggle');
        if (button) button.remove();
        placeholders.get(section)?.after(section);
      }
    });
...
/* How I Work: slow left-to-right arrow sequence; any page click stops it. */
(() => {
  const setupApproachArrowSequence = () => {
    const section = document.querySelector('.approach-viewport');
    if (!section || section.dataset.arrowSequenceReady === 'true') return;
    const cards = [...section.querySelectorAll('.studio-info-card')].slice(0, 3);
    if (cards.length !== 3) return;
    section.dataset.arrowSequenceReady = 'true';
    section.classList.add('arrow-sequence-running');
    let activeIndex = 0;
    let timer = null;
    const show = (index) => cards.forEach((card, cardIndex) => card.classList.toggle('is-arrow-active', cardIndex === index));
    const stop = () => {
      if (timer) window.clearInterval(timer);
      timer = null;
      cards.forEach(card => card.classList.remove('is-arrow-active'));
      section.classList.remove('arrow-sequence-running');
      section.classList.add('arrow-sequence-stopped');
    };
    show(activeIndex);
