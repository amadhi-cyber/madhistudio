...
/* Mobile landing flow: interleave each service heading with its existing featured work. */
(() => {
  ...
  function arrangeMobileServices() {
    ...
    if (!grid) return;
    const cards = [...grid.querySelectorAll(':scope > .sitemap-card')];
    ids.forEach((id, index) => {
      ...
      if (mobileQuery.matches) card.after(section);
      else placeholders.get(section)?.after(section);
    });
